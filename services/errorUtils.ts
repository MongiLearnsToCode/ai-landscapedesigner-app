export const sanitizeError = (error: any): string => {
  ...
  if (message.includes('service temporarily unavailable')) return 'Service temporarily unavailable';
  if (message.includes('api') && message.includes('key')) return 'Service temporarily unavailable';
  if (message.includes('credential') || message.includes('google_cloud_project') || message.includes('google cloud project')) return 'Service temporarily unavailable';
