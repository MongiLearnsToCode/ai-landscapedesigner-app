...
// Convert deployment name to URL if needed
const getConvexUrl = () => {
  ...
  if (url) return url;
  if (deployment) {
    // Convert deployment format to URL
    const deploymentId = deployment.replace('dev:', '');
    return `https://${deploymentId}.convex.cloud`;
  }
  throw new Error('No Convex URL found in environment variables');
};
...
async function handleSubscriptionActive(subscription) {
  ...
  const billingCycle = subscription.price.recurringInterval === 'year' ? 'annual' : 'monthly';
